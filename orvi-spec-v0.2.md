# Orvi Specification v0.2

This is the second executable Orvi specification. It is a backwards-compatible
superset of [`v0.1`](orvi-spec-v0.1.md): it only adds inline constructs, so every
valid `v0.1` document is a valid `v0.2` document and renders identically unless it
uses one of the newly meaningful characters (`` \ ``, `` ` ``, a lone `*`, a
trailing line `` \ ``, a bare `http(s)://` URL, or a colon-decorated table
divider).

A document may mark its version with `orvi: 0.1` or `orvi: 0.2`; both validate
against this parser.
...
Metadata is not rendered as content. v0.2 supports `orvi: 0.1|0.2`,
...
metadata_value  = "0.1" | "0.2" | title_text | language_tag | "ltr" | "rtl" | "auto" ;
