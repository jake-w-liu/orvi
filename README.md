...
// One-shot helpers (each re-parses internally):
...
// Parse once, then render and/or format from the same AST (preferred in
// pipelines and editor integrations):
const source = "See [the docs](https://example.com).";
const ast = parseOrvi(source);
const fragment = renderToHtml(ast, { fullDocument: false });
const pretty = formatOrviFromAst(ast, { source }).formatted;
...
export function Page({ source }: { source: string }) {
  // Primitive option fields are compared by value; memoize `theme` (and any
  // `renderNode` callback) so a new object each render does not re-parse.
  const theme = useMemo(() => ({ colors: { blue: "#0f766e" } }), []);
  return (
    <OrviRenderer
      source={source}
      renderOptions={{ colorScheme: "dark", theme }}
    />
  );
}
```
...
| Import | Provides |
| --- | --- |
| `orvi-lang` | `parseOrvi`, `renderOrvi`, `renderToHtml`, `formatOrvi`, `formatOrviFromAst`, `walk`, `OrviParser`, `defaultCss`, plus the AST/diagnostic types |
