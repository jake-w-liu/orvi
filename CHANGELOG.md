...
- **`formatOrviFromAst`:** format an already-parsed `DocumentNode` without a
  second parse. Pass `{ source }` to keep comment-line and unrecognized-metadata
  drop warnings; AST-only losses (badges, adjacent lists, unrepresentable
  options) are always reported. `formatOrvi` remains the one-shot helper.

### Changed

- **Format-loss diagnostics:** badge / list / option losses are collected in a
  single AST walk instead of three separate walks.
- **`OrviRenderer`:** primitive `renderOptions` fields are compared by value so
  an inline `renderOptions={{ colorScheme: "dark" }}` does not force a re-parse
  every render. Stabilize `theme` and `renderNode` with `useMemo` /
  `useCallback` when needed.
- **HTML escaping:** shared `src/html.ts` used by the renderer and CLI;
  Obsidian runtime now ships `html.js` alongside the other runtime modules.
- **Docs restored:** README, CHANGELOG, specs, stability contract, and related
  markdown that packaging and CI still require (removed earlier by accident
  relative to tests).
- **Lint:** ignore `fuzz/` harnesses (same as other non-library trees).
