...
| --- | --- |
| `orvi-lang` | `parseOrvi`, `renderOrvi`, `renderToHtml`, `formatOrvi`, `formatOrviFromAst`, `walk`, `OrviParser`, `defaultCss`, and the AST / diagnostic / options TypeScript types |
| `orvi-lang/parser` | `parseOrvi`, `OrviParser` |
| `orvi-lang/renderer` | `renderOrvi`, `renderToHtml`, `defaultCss`, `RenderOptions` |
| `orvi-lang/formatter` | `formatOrvi`, `formatOrviFromAst` |
...
- Need custom output? `parseOrvi(source)` gives you the AST; `walk(ast, visit)`
  does a depth-first traversal. Prefer `renderToHtml(ast)` /
  `formatOrviFromAst(ast, { source })` over re-parsing with `renderOrvi` /
  `formatOrvi` when you already hold the AST.
- Need to tweak the HTML? `renderOrvi(source, { renderNode })` (or
  `renderToHtml(ast, { renderNode })`) overrides the HTML for individual block
  nodes (with a `defaultRender` callback for fall-through), or post-process the
  returned string.
...
`renderToHtml`, `formatOrvi`, `formatOrviFromAst`, and `renderOrviArtifact` never throw on it, the
