...
export class OrviParser {
  ...
  static fromSource(source: string, options: ParserOptions = {}): OrviParser {
    ...
    return new OrviParser(
      ...
      lines.map((text, index) => ({ text: expandLeadingTabs(text), line: index + 1 })),
      [],
  ...
  private parseBlockquote(depth: number): BlockquoteNode {
    const start = this.current();
  ...
  private parseList(depth: number): BlockNode {
    const start = this.current();
    ...
    while (!this.isEnd()) {
      ...
      if (ordered && startNumber === undefined) startNumber = marker.number;
      this.index += 1;

      const body: SourceLine[] = [];
      if (marker.content !== "") body.push({ text: marker.content, line: itemLine.line });
      let itemLoose = false;
      ...
      while (!this.isEnd()) {
        ...
        if (inFence) {
          ...
          if (indentWidth(line.text) >= marker.contentCol) {
            ...
            if (isFenceLine(dedented)) inFence = false;
            this.index += 1;
            continue;
          }
        ...
        if (blank) {
          ...
          if (body.length > 0 && next && indentWidth(next.text) >= marker.contentCol) {
            ...
            for (let i = this.index; i < next.index; i += 1) body.push({ text: "", line: this.lines[i]!.line });
        ...
        if (indentWidth(line.text) >= marker.contentCol) {
          const dedented = dedentBy(line.text, marker.contentCol);
          body.push({ text: dedented, line: line.line });
          if (isFenceLine(dedented)) inFence = true;
          this.index += 1;
          continue;
        }
        break;
      }
      if (itemLoose) loose = true;
  ...
  private peekNonBlank(): { text: string; index: number } | undefined {
    let i = this.index;
    while (i < this.lines.length && this.lines[i]!.text.trim() === "") i += 1;
...
function dedentBy(text: string, count: number): string {
  let index = 0;
