...
class OrviParser {
    ...
    isBlockBoundary(line, checkTableAhead = true) {
        const trimmed = line.text.trim();
        if (trimmed === "" || trimmed.startsWith("//"))
            return true;
        if (trimmed.startsWith("```"))
...
function splitTableRow(row) {
    ...
    if (cells.length > 1 && cells[0].trim() === "" && value.startsWith("|"))
        cells.shift();
    if (cells.length > 1 && cells[cells.length - 1].trim() === "" && value.endsWith("|"))
        cells.pop();
...
function indexOfUnescapedBracket(value, start) {
    let from = start;
    while (from < value.length) {
        ...
        if (at < 0)
            return -1;
        let backslashes = 0;
        while (at - 1 - backslashes >= 0 && value[at - 1 - backslashes] === "\\")
            backslashes += 1;
        if (backslashes % 2 === 0)
            return at;
        from = at + 1;
